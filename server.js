...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'q10-genius-backend', timestamp: new Date().toISOString() });
});
...
app.post('/api/q10/webhook/form-completed', (req, res) => {
  ...
  // Here you could: notify via Telegram, update tracking store, etc.
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   Q10 API Key: ${process.env.Q10_API_KEY ? '✓ configured' : '✗ MISSING'}`);
});
